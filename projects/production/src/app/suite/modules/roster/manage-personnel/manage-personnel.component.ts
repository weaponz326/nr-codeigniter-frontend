...
export class ManagePersonnelComponent implements OnInit, AfterViewInit {
  ...
  source: any = {
    ...
    dataFields: [
      { name: 'id', type: 'string' },
      { name: 'worker_id', map: 'worker>id', type: 'string' },
      { name: 'worker_code', map: 'worker>worker_code', type: 'string' },
      { name: 'worker_name', map: 'worker>worker_name', type: 'string' },
  ...
  columns: any[] = [
    { text: 'Worker ID', dataField: 'worker_code', width: "30%" },
    { text: 'Worker Name', dataField: 'worker_name', width: "50%" },
